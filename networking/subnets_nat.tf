...
resource "aws_subnet" "public_nat" {
  for_each = var.public_subnets_nat
...
resource "aws_eip" "nat" {
  for_each = var.single_nat ? { keys(aws_subnet.public_nat)[0] = values(aws_subnet.public_nat)[0] } : aws_subnet.public_nat
...
resource "aws_route_table" "public" {
  for_each = aws_subnet.public_nat
