resource "aws_elasticache_replication_group" "redis" {
  ...
  replication_group_id        = var.global_replication_group_id == null ? "${var.environment}-${var.account}-${var.project}-redis" : "${var.environment}-${var.account}-${var.project}-redis-replica"
  ...
  dynamic "log_delivery_configuration" {
    ...
    tags = merge(
        {
        "Name" = "${var.environment}-${var.account}-${var.project}-redis"
        },
        var.tags
    )
}
...
resource "aws_elasticache_parameter_group" "redis" {
  name        = "${var.environment}-${var.account}-${var.project}-redis-${random_id.redis_pg.hex}"
  family      = var.family
  description = var.description
...
resource "aws_elasticache_subnet_group" "redis" {
  name        = var.global_replication_group_id == null ? "${var.environment}-${var.account}-${var.project}-redis" : "${var.environment}-${var.account}-${var.project}-redis-replica"
...
resource "aws_security_group" "redis" {
  name = "${var.environment}-${var.account}-${var.project}-redis-sg"
  vpc_id      = var.vpc_id
