...
resource "aws_iam_role" "default" {
  count                = local.create_role ? 1 : 0
  name                 = var.capacity_type == "SPOT" ? "${var.environment}-${var.account}-${var.project}-${replace(var.instance_types[0],".","")}-spot-ng-role" : "${var.environment}-${var.account}-${var.project}-${replace(var.instance_types[0],".","")}-ondemand-ng-role" 
...
#   name   = "${aws_eks_node_group.default.node_group_name}-CNI_Policy" #"${module.this.id}-CNI_Policy"
