...
locals {
  ...
  enabled = true
  get_cluster_data = local.enabled ? (local.need_cluster_kubernetes_version || local.need_bootstrap || length(var.associated_security_group_ids) > 0) : false
...
locals {
  ng = {
    ...
    #local.create_role ? join("", aws_iam_role.default.*.arn) : try(var.node_role_arn[0], null)
    ...
    tags = var.tags #local.node_group_tags
...
resource "aws_eks_node_group" "default" {
  ...
  node_group_name = var.capacity_type == "SPOT" ? "${var.environment}-${var.account}-${var.project}-${replace(var.instance_types[0],".","")}-spot-ng" : "${var.environment}-${var.account}-${var.project}-${replace(var.instance_types[0],".","")}-ondemand-ng"
