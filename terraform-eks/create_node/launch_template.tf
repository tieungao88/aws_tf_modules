locals {
  ...
  need_bootstrap = local.enabled ? length(concat(var.kubelet_additional_options, var.after_cluster_joining_userdata
  )) > 0 : false
...
locals {
  ...
  launch_template_id = local.enabled ? (local.fetch_launch_template ? var.launch_template_id[0] : aws_launch_template.default.id) : ""
  launch_template_version = local.enabled ? (length(var.launch_template_version) == 1 ? var.launch_template_version[0] : (
    local.fetch_launch_template ? data.aws_launch_template.this[0].latest_version : aws_launch_template.default.latest_version
  )) : null
...
resource "aws_launch_template" "default" {
  ...
  name            = var.capacity_type == "SPOT" ? "${var.environment}-${var.account}-${var.project}-${replace(var.instance_types[0],".","")}-spot-ng-lt" : "${var.environment}-${var.account}-${var.project}-${replace(var.instance_types[0],".","")}-ondemand-ng-lt"
